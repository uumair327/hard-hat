...
class PerformanceOptimizationReport {
  static final PerformanceOptimizationReport _instance =
  ...
  Map<String, dynamic> generateReport() {
    ...
    return {
      ...
      'performance': {
        'current_metrics':
            _performanceMonitor.currentMetrics?.toString() ??
      ...
      'optimization': {
        ...
        'auto_optimization_enabled':
            _performanceOptimizer.autoOptimizationEnabled,
      },
      'object_pools': {
        'stats': poolStats,
      ...
      if (stats is String && stats.contains('hitRate:')) {
        try {
          ...
          if (hitRateMatch != null) {
            ...
            totalHitRate +=
                hitRatePercent / 100.0; // Convert percentage to decimal
            poolCount++;
          }
        } catch (e) {
  ...
  List<String> _generateRecommendations() {
    ...
    if (currentMetrics == null) {
      recommendations.add(
        'Enable performance monitoring to get detailed recommendations',
      );
    ...
    if (currentMetrics.frameRate < 45.0) {
      recommendations.add(
        'Frame rate is below 45 FPS. Consider reducing visual effects or enabling performance optimizations.',
      );
    } else if (currentMetrics.frameRate < 55.0) {
      recommendations.add(
        'Frame rate is below optimal. Monitor for performance degradation.',
      );
    }
    ...
    if (memoryMB > 150) {
      recommendations.add(
        'Memory usage is high (${memoryMB.toStringAsFixed(1)}MB). Consider clearing caches or reducing object pool sizes.',
      );
    } else if (memoryMB > 100) {
      recommendations.add(
        'Memory usage is moderate (${memoryMB.toStringAsFixed(1)}MB). Monitor for memory leaks.',
      );
    }
    ...
    if (currentMetrics.cpuUsage > 80.0) {
      recommendations.add(
        'CPU usage is high (${currentMetrics.cpuUsage.toStringAsFixed(1)}%). Consider reducing collision checks or particle counts.',
      );
    }
      ...
      if (stats is String) {
        try {
          ...
          if (hitRateMatch != null) {
            ...
            if (hitRate < 0.7) {
              recommendations.add(
                '$poolName pool has low hit rate (${hitRatePercent.toStringAsFixed(1)}%). Consider increasing pool size.',
              );
            }
          }
          ...
          if (activeMatch != null && maxSizeMatch != null) {
            ...
            if (active > maxSize * 0.9) {
              recommendations.add(
                '$poolName pool is nearly full. Consider increasing max size or reducing usage.',
              );
            }
          }
        } catch (e) {
    ...
    if (!_performanceOptimizer.autoOptimizationEnabled) {
      recommendations.add(
        'Auto-optimization is disabled. Enable it for automatic performance adjustments.',
      );
    }
  ...
  Map<String, dynamic> _analyzeFrameTime(PerformanceMetrics? metrics) {
    ...
    if (frameTimeRatio <= 1.0) {
      ...
      status = 'Fair';
      recommendation =
          'Frame time may cause stuttering. Consider optimizations.';
    } else {
      status = 'Poor';
  ...
  Map<String, dynamic> _analyzeOptimizationHistory() {
    ...
    if (history.isEmpty) {
      return {
        ...
        'recommendation':
            'Monitor performance and apply optimizations as needed',
      };
    }
    ...
    return {
      ...
      'success_rate':
          '${((successfulOptimizations / history.length) * 100).toStringAsFixed(1)}%',
      'total_performance_improvement':
          '${totalImprovements.toStringAsFixed(1)}%',
      'recent_optimizations': history
          .take(5)
          .map(
            (r) => {
  ...
  String exportReportAsString() {
    ...
    buffer.writeln(
      '  Status: ${performance['is_performance_good'] ? 'Good' : 'Needs Attention'}',
    );
    buffer.writeln();
    ...
    buffer.writeln(
      '  Average Efficiency: ${(pools['efficiency'] * 100).toStringAsFixed(1)}%',
    );
    buffer.writeln();
