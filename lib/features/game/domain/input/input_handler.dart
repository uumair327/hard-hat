...
class KeyboardInputHandler extends InputHandler {
  final StreamController<InputEvent> _inputController =
  ...
  void handleKeyDown(LogicalKeyboardKey key) {
    ...
    switch (command) {
      ...
      case InputCommand.pause:
        _inputController.add(
          PauseInputEvent(source: InputSource.keyboard, timestamp: timestamp),
        );
        break;
      default:
        break;
...
class GamepadInputHandler extends InputHandler {
  ...
  void handleButtonDown(int buttonId) {
    ...
    switch (command) {
      ...
      case InputCommand.pause:
        _inputController.add(
          PauseInputEvent(source: InputSource.gamepad, timestamp: timestamp),
        );
        break;
      default:
        break;
...
class CrossPlatformInputHandler extends InputHandler {
  ...
  Future<void> initialize() async {
    ...
    _keyboardSubscription = _keyboardHandler.inputStream.listen(
      _handleInputEvent,
    );
    _touchSubscription = _touchHandler.inputStream.listen(_handleInputEvent);
    _gamepadSubscription = _gamepadHandler.inputStream.listen(
      _handleInputEvent,
    );
  }
