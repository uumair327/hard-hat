...
class HardHatGame extends FlameGame
  ...
  Future<void> _initializeDomainLayer() async {
    try {
      debugPrint('HardHatGame: Initializing real game controller');
      _gameController = manual_di.getIt<IGameController>();
      await _gameController.initializeGame();
      debugPrint('HardHatGame: Domain layer initialized successfully');
    } catch (e) {
      debugPrint(
        'HardHatGame: Error initializing domain layer, falling back to dummy: $e',
      );
  ...
  void update(double dt) {
    ...
    final clampedDt = dt.clamp(
      0.0,
      1.0 / 20.0,
    ); // Max 20 FPS equivalent for stability
    ...
    if (_totalFrameTime - _lastPerformanceReport >= performanceReportInterval) {
      ...
      debugPrint(
        'Game Performance: ${fps.toStringAsFixed(1)} FPS (avg frame time: ${(avgFrameTime * 1000).toStringAsFixed(2)}ms)',
      );
  ...
  IGameStateManager get gameStateManager =>
      manual_di.getIt<IGameStateManager>();
  ...
  IStateTransitionSystem? get stateTransitionSystem =>
      manual_di.getIt<IStateTransitionSystem>();
  ILevelManager? get levelManager =>
      manual_di.getIt.isRegistered<ILevelManager>()
      ? manual_di.getIt<ILevelManager>()
      : null;
  ISaveSystem? get saveSystem => manual_di.getIt.isRegistered<ISaveSystem>()
      ? manual_di.getIt<ISaveSystem>()
      : null;
  ...
  IPauseMenuManager? get pauseMenuManager =>
      manual_di.getIt.isRegistered<IPauseMenuManager>()
      ? manual_di.getIt<IPauseMenuManager>()
      : null;
